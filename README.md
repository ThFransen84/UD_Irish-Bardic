...
While traces of the prescriptive usage of case forms are salient in our poem, we see a confusion between dative and accusative constructions governed by prepositions typical of the post-Classical period. In addition, case forms are often conflated, such as with acc./dat.sg _airdrígh_ (`bardic_1387-3`) “high-king” for expected nom.sg _airdrí_ (Mac Cárthaigh 2013, pp. 47–48). For this reason, the accusative case feature, which does not exist in the modern language, has not been used; the dative is only assigned in cases where the noun has a distinctive dative form and is not also used as a nom.sg in our poem. Likewise, where an original dat.sg exists as a variant nom.sg in the modern language, as per Ó Dónaill (1977), the noun is tagged as a nom.sg, e.g. _caroid_ (_caraid_) “friend” (`bardic_1387-17`) for standard _cara_. As Scannell (2022, p. 9) has pointed out, current treebank annotation conventions might need to be re-evaluated based on future discussions with other Irish treebank maintainers. In the meantime, any feedback from language specialists is greatly welcomed. 
